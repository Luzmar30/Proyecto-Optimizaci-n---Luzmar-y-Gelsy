...
def simular_configuracion(dn_pulg, grado_acero, caudal_MMscfd, num_estaciones,
    ...
    if diametro_int_pulg <= 0:
        # Esto indica dimensiones de tubería físicamente imposibles (espesor de pared >= DE).
        # En lugar de usar DE como DI, se debería invalidar esta configuración.
        raise ValueError(f"Diámetro interno calculado <= 0 para DN_pulg={dn_pulg}. Revise datos de tubería.")
...
for i in range(N_est):
    ...
    for j in range(50):
        ...
        P = np.sqrt(max(0, P_inicio**2 - fraccion * res["dp2_tramo"])) # Asegura que el argumento de sqrt no sea negativo
        distancias.append(x)
        presiones.append(P)
