...
# CSS para fondo oscuro, botones lilas/cyan, métricas y alertas
st.markdown("""
<style>
...
def weymouth_dp(caudal_MMscfd, longitud_km, diametro_pulg, gravedad, temp_K, Z):
    """
    Ecuación de Weymouth según enunciado:
    Δ(P²) = 433.5 * (Q/E)² * (L * γ * T * Z) / D^5.33
    L en millas, T en Rankine, D en pulgadas, Q en MMscfd.
    Se asume eficiencia E = 1 (no se especifica).
    """
    L_millas = longitud_km * 0.621371          # km → millas
    T_R = temp_K * 9.0/5.0                    # K → °R
    d_pulg = float(diametro_pulg)
    dp2 = 433.5 * (caudal_MMscfd)**2 * (L_millas * gravedad * T_R * Z) / (d_pulg**5.33)
    return dp2
...
def simular_configuracion(dn_pulg, grado_acero, caudal_MMscfd, num_estaciones,
    ...
    Retorna diccionario con resultados y banderas de alerta.
    """
    ...
    # Calcular presiones desde el final hacia atrás
...
# =============================================================================
# OPTIMIZACIÓN: BÚSQUEDA DE MÍNIMO TAC (usando caudal base 500 MMscfd)
# =============================================================================
...
def main():
    ...
    # Desglose de costos (gráfico de barras y de sectores)
